      ...
      <div className="hidden lg:flex flex-col gap-10">
        ...
        >
          {journey_options.map((items) => (
            <div
              key={items.id}
              className="h-auto w-full rounded-3xl bg-[#F2F2F2] p-4 flex flex-row gap-4"
            >
              <img
                className="w-6/12 h-60 rounded-3xl object-cover"
                src={items.image}
              />
          ...
          <div className="flex flex-wrap gap-3 justify-between mt-2">
            {buttonoptions.map((items) => (
              <button
                ...
                key={items}
                onClick={() => setbtn(items)}
                className={`text-sm font-poppins justify-between rounded-3xl px-8 py-3 transition-colors duration-800 ${
                  btn === items
                    ? "bg-[#000000] text-white"
                    : "border border-[#989898] text-black hover:bg-black hover:text-white"
                }`}
